...
uint16_t Adafruit_NeoPixel::stripLen;
void * Adafruit_NeoPixel::frameBuffer;
void * Adafruit_NeoPixel::drawBuffer;

static const uint8_t ones = 0xFF;
static volatile uint8_t update_in_progress = 0;
static uint32_t update_completed_at = 0;

Adafruit_NeoPixel::Adafruit_NeoPixel(uint16_t n, void *frameBuf, void *drawBuf, uint8_t p, uint8_t t) : numLEDs(n), numBytes(n * 24), pinCount(p), type(t)
{
  stripLen = n;
...
Adafruit_NeoPixel::~Adafruit_NeoPixel()
{
  ...
  if (drawBuffer != NULL) free(drawBuffer);
}
...
void Adafruit_NeoPixel::begin(void) {
  ...
  frequency = (type & NEO_SPDMASK) == NEO_KHZ400 ? 400000 : 800000;
...
uint8_t Adafruit_NeoPixel::busy(void) {
...
void Adafruit_NeoPixel::setPixelColor(uint32_t num, uint8_t r, uint8_t g, uint8_t b) {
  // if(n < numLEDs) {
  //   uint8_t
  //     r = (uint8_t)(c >> 16),
  //     g = (uint8_t)(c >>  8),
  ...
  setPixelColor(num, Color(r, g, b));
}

// Set pixel color from 'packed' 32-bit RGB color:
void Adafruit_NeoPixel::setPixelColor(uint32_t num, uint32_t color) {
  //   if(n < numLEDs) {
  //     uint8_t
  //       r = (uint8_t)(c >> 16),
  //       g = (uint8_t)(c >>  8),
...
uint32_t Adafruit_NeoPixel::getPixelColor(uint32_t num) {
